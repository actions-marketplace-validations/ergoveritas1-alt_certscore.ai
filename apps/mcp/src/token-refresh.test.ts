...
test("authenticated HTTP sessions rotate credentials, isolate identities and preserve request-local fan-out", { timeout: 30_000 }, async () => {
  ...
  const api = createServer(async (req, res) => {
    ...
    if (path === "/api/v2/scans" && req.method === "POST") {
      res.writeHead(202, { "content-type": "application/json" });
      res.end(JSON.stringify({ type: "certscore_scan_job", scanId, status: "queued", url: "https://fixture.test/", retryAfterSeconds: 5 }));
      return;
    }
    if (path === `/api/v2/scans/${scanId}`) {
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ type: "certscore_scan", scanId, status: "completed", domain: "fixture.test", score: 92 }));
      return;
    }
    ...
    const bundleId = [scanId, ...concurrentIds].find(id => req.url?.includes(id));
    if (bundleId) {
  ...
  async function post(bearer: string | null, session: string | null, method: string, params: unknown = {}, path = "/mcp") {
    return fetch(origin + path, { method: "POST", headers: {
      "content-type": "application/json", accept: "application/json, text/event-stream", origin: "https://www.cursor.com",
  ...
  try {
    ...
    // Successful creation and retained reads must be MCP results, not merely HTTP 200 errors.
    for (const [name, args] of [
      ["certscore_scan_site", { url: "https://fixture.test/" }],
      ["certscore_get_scan_status", { scanId }],
      ["certscore_get_scan_bundle", { scanId }],
    ] as const) {
      const response = await post(b, session, "tools/call", { name, arguments: args });
      assert.equal(response.headers.get("access-control-allow-origin"), "https://www.cursor.com");
      const body = await expectStatus(response, 200);
      const envelope = JSON.parse(body.startsWith("event:") || body.startsWith("data:")
        ? body.split("\n").find(line => line.startsWith("data: "))!.slice(6) : body);
      assert.equal(envelope.error, undefined, body);
      assert.notEqual(envelope.result.isError, true, body);
      const contract = certScoreMcpToolContracts.find(contract => contract.name === name)!;
      assert.ok(contract.outputSchema.safeParse(envelope.result.structuredContent).success, body);
      assert.equal(envelope.result.structuredContent.scanId, scanId);
    }
