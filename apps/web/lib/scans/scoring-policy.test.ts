...
test("approved registry has exactly the reviewed rows and caps", () => {
  assert.equal(SCORING_RULES.length, 18);
  assert.equal(new Set(SCORING_RULES.map(rule => rule.id)).size, 18);
  ...
  assert.equal(SCORING_RULES.filter(rule => rule.siteWide).length, 9);
