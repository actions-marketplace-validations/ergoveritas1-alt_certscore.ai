...
export function ServiceResourceRows({ row, pageName, scenario = "pre_consent", collapseVersion = 0, resourceContext, serviceContext, nested = false, nestingDepth }: { nestingDepth?: number; nested?: boolean; serviceContext?: ResourceContext["context"]; resourceContext?: ResourceContext; collapseVersion?: number; row: Resource; pageName: (id: string) => string; scenario?: ApiRuntimeEvidenceGraph["scenario"] }) {
  ...
  return <><tr className={`h-14 border-b border-zinc-100 ${open ? "bg-sky-50/40" : "hover:bg-zinc-50"}`}>
    ...
    <td className="w-14 px-2 text-center text-xs font-medium tabular-nums text-slate-600" title="Retained observations for this resource, including repeats">{row.eventCount}</td>
    ...
    <td className={cell}><InventoryPurposeList purposes={resourcePurposeLabels(context?.identity, row.kind, row.purposes)} relationships={row.relationships}/>{occurrence.details.resourceRole === "video_ad_sdk" ? <span className="mt-1 block text-xs text-slate-500">Video advertising SDK</span> : null}</td>
    ...
    <td className={`${cell} whitespace-nowrap`}>{row.relationships.map(value => value === "first_party" ? "First party" : value === "third_party" ? "Third party" : value.replaceAll("_", " ")).join(", ") || "Unknown"}</td>
...
function LinkedRow({ graph, link, path, depth, pages, indentBase }: { indentBase: number; graph: ApiRuntimeEvidenceGraph; link: Link; path: string[]; depth: number; pages: string[] }) {
  ...
  return <><tr data-service-relationship={node.id} className="h-11 border-t border-slate-100 bg-slate-50/50 hover:bg-sky-50/50">
    ...
    <td className={cell}>{node.classification?.purpose ? <InventoryPurposeChip purpose={resourcePurposeLabels(node.classification, node.kind, [node.classification.purpose])[0]!}/> : <span className="text-slate-400" title="Purpose not retained">—</span>}</td>
