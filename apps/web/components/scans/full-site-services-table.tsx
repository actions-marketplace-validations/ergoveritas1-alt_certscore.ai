...
import { INVENTORY_CLASSIFICATION_ORDER } from "../../lib/scans/inventory-resource-semantics";
...
const priorityOrder: readonly string[] = INVENTORY_CLASSIFICATION_ORDER;
export function summarizeService(resources: Service["resources"]) {
  ...
  return { priority, priorityCount: priority ? resources.filter(row => row.inventoryEvidence === priority).length : 0,
    ...
    relationship: first && third ? "Mixed" : first ? "First party" : third ? "Third party" : undefined,
...
function ServiceDomains({ domains }: { domains: string[] }) {
  const id = useId();
  if (!domains.length) return <span title="Unavailable">—</span>;
...
export function FullSiteServices({ collapseVersion = 0, services, pageName, pageChoices, homepageGraph, scenario = "pre_consent" }: { collapseVersion?: number; scenario?: ApiRuntimeEvidenceGraph["scenario"]; services: Service[]; pageName: (id: string) => string; pageChoices: FullSiteReportResponse["pageChoices"]; homepageGraph?: ComponentProps<typeof InventoryResourceProvider>["projection"] }) {
  ...
  return <section aria-label="Services inventory" className="min-w-0 bg-white">
    <div>
      ...
      <div id={unattributedHelpId} popover="auto" role="dialog" aria-label="About unresolved loading origin" className="m-auto w-[min(24rem,calc(100vw-2rem))] rounded-xl border border-slate-200 bg-white p-4 text-xs text-slate-600 shadow-xl">
        <div className="mb-3 flex items-center justify-between gap-3"><h3 className="font-semibold text-slate-900">Loading origin unresolved</h3><button type="button" popoverTarget={unattributedHelpId} popoverTargetAction="hide" aria-label="Close loading origin explanation" className="rounded px-2 py-1 hover:bg-slate-100">Close</button></div>
