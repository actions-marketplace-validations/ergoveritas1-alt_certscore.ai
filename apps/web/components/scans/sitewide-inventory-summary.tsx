...
import { inventoryTypeBreakdown, INVENTORY_CLASSIFICATION_ORDER, INVENTORY_CLASSIFICATION_LABELS, INVENTORY_CLASSIFICATION_DESCRIPTIONS } from "../../lib/scans/inventory-resource-semantics";
...
function Mix({ title, rows, updating }: { updating: boolean; title: string; rows: Array<{ label: string; count: number }> }) {
  ...
  const evidenceOrder: readonly string[] = INVENTORY_CLASSIFICATION_ORDER;
  ...
  return <section className="min-w-0 bg-white px-3 py-2">
    ...
    <ul id={listId} className="mt-1 max-h-44 w-full min-w-0 space-y-0 overflow-auto text-[11px] leading-[14px]" aria-label={`${title}: ${total} resource identities`}>
      {visible.map(row => <li key={row.label} className="flex items-start gap-1.5"><span aria-hidden="true" className="mt-1 h-2 w-2 shrink-0 rounded-full" style={{ background: row.color }} /><span title={title === "Purpose mix" ? inventoryPurposeTitle(row.label) : INVENTORY_CLASSIFICATION_DESCRIPTIONS[row.label]} className="min-w-0 flex-1">{INVENTORY_CLASSIFICATION_LABELS[row.label] ?? (row.label === "first_party" ? "First party" : row.label === "third_party" ? "Third party" : row.label.replaceAll("_", " ").replace(/^./, letter => letter.toUpperCase()))}</span><strong className="shrink-0 tabular-nums"><ScanLiveValue value={row.count} active={updating} /></strong></li>)}
...
export function SitewideInventorySummary({ mix, updating = false }: { updating?: boolean; mix: FullSiteReportResponse["inventoryMix"] }) {
  const total = mix.type?.reduce((sum, row) => sum + row.count, 0);
  const typeRows = mix.type ? inventoryTypeBreakdown(mix.type) : undefined;
  const composition = typeRows?.filter(row => row.count > 0).map(row => `${row.count.toLocaleString()} ${row.label.toLowerCase()}`).join(" + ");
  return <section aria-label="Sitewide resource inventory" className="my-3 border-y border-zinc-200 bg-white">
    <p className="px-3 pt-2 text-[10px] text-zinc-500">{total != null ? `${total.toLocaleString()} distinct resources${composition ? ` = ${composition}` : ""}. Services group these resources.` : "Distinct resources · Services group related resources."}</p>
    <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,9rem),1fr))] gap-px bg-zinc-200">{typeRows ? <Mix updating={updating} title="Type mix" rows={typeRows} /> : null}<Mix updating={updating} title="Evidence mix" rows={mix.evidence} /><Mix updating={updating} title="Purpose mix" rows={mix.purpose} /><Mix updating={updating} title="Site relationship" rows={mix.relationship} /></div>
  </section>;
}
