...
import { resolveBrowserCustomer } from "../../../../../server/marketplace-browser/aws";
import { createBrowserClaim } from "../../../../../server/marketplace-browser/repository";
import { BROWSER_CLAIM_COOKIE, BROWSER_PATH } from "../../../../../server/marketplace-browser/config";
import { admitMarketplaceRequest, boundedText } from "../../../../../server/marketplace/http";

export const dynamic = "force-dynamic";
export async function POST(request: Request) {
  if (!admitMarketplaceRequest("browser-register", 30)) return new Response("Please retry in a minute.", { status: 429, headers: { "Retry-After": "60" } });
  try {
    ...
    const claim = await createBrowserClaim(await resolveBrowserCustomer(token));
    const response = NextResponse.redirect(new URL(BROWSER_PATH, "https://certscore.ai"), 303);
    response.cookies.set(BROWSER_CLAIM_COOKIE, claim, { httpOnly: true, secure: true, sameSite: "lax", path: "/", maxAge: 1800 });
...
export function GET() { return NextResponse.redirect(new URL(BROWSER_PATH, "https://certscore.ai"), 303); }
