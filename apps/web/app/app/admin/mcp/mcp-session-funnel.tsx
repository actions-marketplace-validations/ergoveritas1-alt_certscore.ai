...
export function McpSessionFunnel({ data, followUpMinutes, params }: {
  ...
  return <Card>
    ...
    <CardContent className="space-y-4">
      ...
      <p className="text-xs text-slate-500">{data.outside_cohort_calls.toLocaleString()} visible calls have no matching connection in this cohort, including {data.missing_session_calls.toLocaleString()} without a session identifier. {data.unlinked_initializations ?? 0} initialization events have no linkable session. They remain outside session conversion rates. Default traffic filters exclude known internal/QA activity but retain unknown historical activity without calling it verified external traffic. Missing initialization, old sessions and shared/rotating identifiers limit coverage; sessions are not unique agents.</p>
