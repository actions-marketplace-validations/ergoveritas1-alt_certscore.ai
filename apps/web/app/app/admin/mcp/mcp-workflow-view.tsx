...
export function McpWorkflowView({ followUpMinutes, events, totalEvents, period, traffic, client, surface, source, search, purpose, page, pageSize }: {
  followUpMinutes: number; events: McpWorkflowEvent[]; totalEvents: number; period: McpDiscoveryPeriod; traffic: AdminTrafficScope;
  ...
  return <div className="space-y-4">
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <details className="rounded-lg border p-3"><summary className="cursor-pointer font-semibold">Capture health and recovery diagnostics</summary><section aria-label="Capture health" className="rounded-lg border p-3">
...
| stats count(*) as started_requests, sum(accepted) as accepted_events, sum(failed) as failed_events, sum(if(accepted = 0 and failed = 0, 1, 0)) as unresolved_requests`}</pre><p>Select the hosted MCP log group and a window with time for calls to finish. This cohort covers tool-handler starts; pre-handler access/quota blocks and initialization are separate. Unresolved includes in-flight calls, process exits and missing logs.</p></details>
        ...
        <div className="overflow-x-auto rounded-lg border"><table className="min-w-[760px] w-full text-left text-sm">
          ...
          <tbody className="divide-y">{visibleGroups.map(group => <tr key={group.key} className="align-top">
            ...
            <td className="p-3"><p className="font-semibold">{group.stage}</p>{group.lightMetrics ? <div className="mt-2 text-xs">
              <p>Early caller polls: {group.lightMetrics.earlyPolls}/{group.lightMetrics.measuredPolls} measurable</p>
              <p>Caller polls after observed completion: {group.lightMetrics.afterCompletion}</p>
              <p>Internal preview reads: {group.lightMetrics.internalReadsKnown ? group.lightMetrics.internalReads : "Not recorded"}</p>
              <p>First preview/completed response: {group.lightMetrics.firstUsefulMs === null ? "Not recorded" : `${group.lightMetrics.firstUsefulMs / 1000}s`}</p>
              <p>Completed bundle response: {group.lightMetrics.firstBundleMs === null ? "Not recorded" : `${group.lightMetrics.firstBundleMs / 1000}s`}</p>
              <p>Timing uses captured request starts and generated responses in this session/scan sample; it does not measure client receipt.</p>
            </div> : null}<p className="mt-1 text-xs">{group.scanRequest ? `${label(group.scanRequest.scan_decision)} scan request` : "No scan request in this group"}</p><p className="text-xs">{group.statusCalls} status calls · {group.bundleCalls} bundle calls</p>{group.firstBundleSeconds !== null ? <p className="text-xs">{group.firstBundleSeconds.toFixed(1)}s from scan response to first bundle</p> : null}<p className="mt-2 text-xs text-slate-500">{formatAdminDateTime(group.last.occurred_at)}</p></td>
