...
import { discoveryBehavior, discoveryDiagnostic, MCP_DISCOVERY_PERIODS, mcpClientHref, type McpDiscoveryClient, type McpDiscoveryPeriod } from "../../../../lib/admin/mcp-discovery";
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="w-full min-w-[900px] text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100">{data.items.map(row => <tr className="align-top hover:bg-slate-50" key={JSON.stringify([row.client_name, row.surface, row.source])}>
                ...
                <td className="px-3 py-3"><span className={`inline-block rounded-full px-2 py-1 ${row.tool_calls > 0 ? "bg-emerald-50 text-emerald-800" : "bg-sky-50 text-sky-800"}`}>{discoveryBehavior(row)}</span><p className="mt-2 max-w-64 text-slate-500">{discoveryDiagnostic(row)}</p></td>
                ...
                <td className="px-3 py-3"><p>{row.tool_calls.toLocaleString()} / <span className={row.tool_errors ? "text-rose-700" : ""}>{row.tool_errors.toLocaleString()}</span></p><p>{row.validation_errors ?? 0} invalid requests / protocol probes · {row.execution_errors ?? 0} other execution errors</p><p className="text-slate-500">Invalid input does not establish malicious intent or internal ownership.</p><p className={`mt-1 ${row.quota_hits ? "font-semibold text-amber-800" : "text-slate-500"}`}>{row.quota_hits} rate limits · {row.http_429} HTTP 429</p>{row.error_codes?.length ? <p className="mt-1 max-w-52 break-words text-rose-700">{row.error_codes.join(", ")}</p> : null}</td>
