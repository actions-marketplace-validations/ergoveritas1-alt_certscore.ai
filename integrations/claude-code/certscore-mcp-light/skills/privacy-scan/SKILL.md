...
When the server advertises the optional `taskContext` argument on `certscore_scan_site`, include `integrationId="claude-code-certscore-mcp-light"` and `integrationVersion="0.2.22"` with `skillVersion="2026-09-08.1"`. If the user's stated task clearly supplies a purpose, include one of `prelaunch_review`, `vendor_review`, `tracking_check`, `consent_gpc_check`, `policy_review`, or `recheck`; otherwise leave purpose omitted or `unknown`. These are caller-declared research hints and do not change scanning.
