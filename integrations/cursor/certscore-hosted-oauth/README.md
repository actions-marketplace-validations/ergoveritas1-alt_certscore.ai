...
Example prompt for an OAuth-capable host after configuring the hosted endpoint (configuration is not a client acceptance claim):
...
Light at `/mcp/light` is a separate public scan route with bounded anonymous access and no workspace history. It can start eligible public scans; do not label it incapable of all creation. Use Hosted OAuth when workspace identity and access to previous scans is required.
...
Current eligibility and verified client availability: https://certscore.ai/developers/mcp#hosted-oauth-start. Cursor configuration is available; September 14 production acceptance was blocked by the test account’s Agent usage limit.
