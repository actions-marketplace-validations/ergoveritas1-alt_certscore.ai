...
export function withResponseGuidance(tool: string, input: unknown, result: CallToolResult, now = Date.now()): CallToolResult {
  ...
  else if (tool === 'certscore_get_report_evidence_page' && typeof pagination.nextCursor === 'string') nextAction = { tool, arguments: { scanId, cursor: pagination.nextCursor }, instruction: 'Fetch the next report evidence page. Keep the same snapshot until export is complete.' };
