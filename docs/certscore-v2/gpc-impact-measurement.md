...
The session finalizer independently recognizes exact full-URL `historyApi` no-ops on its tracked committed main document. It retains up to six typed invalidation reasons observed after terminal readback starts. Document requests/commits, route or fragment changes, unknown history identity, crashes, and closure still invalidate pending proof. Change-then-return cannot revive that proof. This adds no second terminal readback, browser call, wait, retry, or later publication; cancellation and immutable final packets remain authoritative. Emochi/VirusTotal's historical generation changes are not retrospectively reclassified as harmless.

Expected incremental compute and storage for this follow-up is below $0.75/month at 100,000 scans and 30-day retention, disclosed before implementation. Storage allowance assumes at most 16 KiB per passive comparison lane and up to three retained copies (approximately 9.8 GB/month worst-case added steady-state bytes); expected compute allowance is below $0.50/month. Actual restoration is usually smaller and many scans require none. No deployment or public scan is part of implementation verification; strict contract consumers must be updated before new producers in a future release.
